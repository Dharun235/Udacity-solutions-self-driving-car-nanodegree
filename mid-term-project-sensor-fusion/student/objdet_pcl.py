...
import sys
PACKAGE_PARENT = '..'
import loop_over_dataset as main
...
def bev_from_pcl(lidar_pcl, configs):
   ...
   # Step 3: Extract top-most points and store counts
    unique_x_y, idx_height_unique, counts = np.unique(lidar_pcl_top[:, :2], axis=0, return_index=True, return_counts=True)
    lidar_pcl_top_unique = lidar_pcl_top[idx_height_unique]

    ## step 4 : assign the intensity value of each unique entry in lidar_top_pcl to the intensity map 
    ##          make sure that the intensity is scaled in such a way that objects of interest (e.g. vehicles) are clearly visible    
    ##          also, make sure that the influence of outliers is mitigated by normalizing intensity on the difference between the max. and min. value within the point cloud
    intensity_min = np.amin(lidar_pcl_top[:, 3])
    intensity_max = np.amax(lidar_pcl_top[:, 3])
    intensity_range = intensity_max - intensity_min

    ### Ensure intensity range is nonzero to avoid division by zero
    if intensity_range != 0:
        intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = (lidar_pcl_top[:, 3] - intensity_min) / intensity_range
    else:
    ### If intensity range is zero, set all intensity values to 0.5
        intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = 0.5
...
# sort points such that in case of identical BEV grid coordinates, the points in each grid cell are arranged based on their intensity
     # Sort points based on intensity within each grid cell
    idx_intensity_sort = np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    lidar_pcl_cpy_sorted = lidar_pcl_cpy[idx_intensity_sort]

    # Keep only one point per grid cell
    _, unique_indices = np.unique(lidar_pcl_cpy_sorted[:, :2], axis=0, return_index=True)
    lidar_pcl_unique = lidar_pcl_cpy_sorted[unique_indices]

    # Create intensity map for visualization
    intensity_map = np.zeros((configs.bev_height + 1, configs.bev_width + 1))
    intensity_map[np.int_(lidar_pcl_unique[:, 0]), np.int_(lidar_pcl_unique[:, 1])] = lidar_pcl_unique[:, 3]

    # Scale intensity for visualization
    intensity_map_visual = (intensity_map - np.min(intensity_map)) / (np.max(intensity_map) - np.min(intensity_map))

    # Convert to uint8 for visualization using OpenCV
    intensity_map_visual = (intensity_map_visual * 255).astype(np.uint8)
    ...
    height_map_visual = (height_map * 255).astype(np.uint8)  # Scale for visualization
    cv2.imshow("Height Map", height_map_visual)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
